...
test.describe('Analytics Page Comprehensive Testing', () => {
  ...
  test('Analytics Dashboard - Data Elements and Visualization', async ({ page }) => {
  ...
  test('Responsive Design Testing', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      if (viewport.width <= 768) {
        const mobileElements = await page.locator('.mobile-menu, [data-testid*="mobile"], .hamburger, .menu-toggle').count();
  ...
  test('Performance and Web Vitals Analysis', async ({ page }) => {
    ...
    const performanceMetrics = await page.evaluate(() => {
      ...
      return {
        ...
        domComplete: perfData ? Math.round(perfData.domComplete - perfData.navigationStart) : 0,
    ...
    const webVitalsContent = await page.evaluate(() => {
      ...
      return {
        foundKeywords,
        hasPerformanceData: bodyText.includes('ms') || bodyText.includes('score') || bodyText.includes('%'),
        hasCharts: document.querySelectorAll('canvas, svg, .chart').length > 0,
        hasMetrics: document.querySelectorAll('.metric, [data-testid*="metric"]').length > 0
      };
    });
  ...
  test('Analytics Page Content and Features Summary', async ({ page }) => {
    ...
    const pageInfo = await page.evaluate(() => {
      ...
      return {
        ...
        hasLoadingStates: document.body.textContent?.toLowerCase().includes('loading') || false,
    ...
    const summary = {
      ...
      hasInteractiveElements: pageInfo.elements.buttons.count > 0 || pageInfo.elements.links.count > 0,
