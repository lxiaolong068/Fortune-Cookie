...
export function isWebVitalMetric(obj: unknown): obj is WebVitalMetric {
  return (
    ...
    ['CLS', 'INP', 'FCP', 'LCP', 'TTFB', 'FID'].includes((obj as WebVitalMetric).name) &&
    typeof (obj as WebVitalMetric).value === 'number' &&
    typeof (obj as WebVitalMetric).delta === 'number' &&
    ['good', 'needs-improvement', 'poor'].includes((obj as WebVitalMetric).rating)
  )
}
...
export function isAnalyticsEvent(obj: unknown): obj is AnalyticsEvent {
  return (
    ...
    ['user_action', 'business_event', 'performance', 'error'].includes((obj as AnalyticsEvent).type) &&
...
export function isFortuneRequest(obj: unknown): obj is FortuneRequest {
  ...
  if (request.theme && !['funny', 'inspirational', 'love', 'success', 'wisdom', 'random'].includes(request.theme)) {
    return false
  }
  
  if (request.mood && !['positive', 'neutral', 'motivational', 'humorous'].includes(request.mood)) {
    return false
  }
...
export function createErrorResponse(error: string, meta?: Record<string, unknown>): ApiErrorResponse {
