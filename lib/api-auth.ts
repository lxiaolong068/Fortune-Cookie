...
/**
 * AI generation rate limit exceeded response
 *
 * Creates a user-friendly error response when hourly AI generation limit is reached.
 *
 * @param reset - Unix timestamp when the rate limit resets
 * @returns Error response object with wait time and suggestions
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export function getAIRateLimitErrorResponse(reset: number) {
  const waitMinutes = Math.ceil((reset - Date.now()) / 60000);

  return {
    success: false,
    error: "AI generation limit exceeded",
    message: `You have reached your hourly AI fortune generation limit. Please try again in ${waitMinutes} minute${waitMinutes !== 1 ? "s" : ""}.`,
    suggestion: "While waiting, browse our collection of pre-seeded fortunes!",
    browseUrl: "/browse",
    reset,
